												...
												<li class="flex py-6 border-b-2 border-gray-200" v-for="data in cart">
													...
													<div class="flex flex-col flex-1 ml-4">
														...
														<div class="flex items-end justify-between flex-1 text-sm">
															...
															<div class="flex">
																<button @click.prevent="$store.dispatch('cart/removeCart', data.id_cart)" type="button" class="font-medium text-indigo-600 hover:text-indigo-400">Remove</button>
															</div>
														</div>
													</div>
												</li>
...
export default {
	...
	methods: {
		...
		qty(data) {
			let withoutUkuran = data.qty_produk + data.sku.produk.satuan_produk;
			let withUkuran = data.ukuran + data.sku.produk.satuan_produk + " - " + data.qty_produk + " pcs";
			return data.sku.produk.satuan_produk == "m2" ? withUkuran : withoutUkuran;
		},
		checkout() {
