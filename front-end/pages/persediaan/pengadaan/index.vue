							...
							<tr class="text-sm">
								...
								<td class="w-48 p-1 py-3">
									<div class="flex flex-wrap gap-1">
										...
										<template v-if="role != 'Operator Inventory'">
										...
										<template v-if="role != 'Manager Inventory'">
				...
				<div class="grid grid-cols-12 col-span-12 gap-4" v-for="(data, index) in pengadaan">
					<div class="col-span-12 sm:col-span-6">
						...
						<select class="form-input form-input-lg" v-model="pengadaan[index].id_bahan_baku">
							...
							<option :hidden="pengadaan.find((item) => item.id_bahan_baku == data.id_bahan_baku)" :value="{id: data.id_bahan_baku, satuan: data.satuan_bahan_baku}" v-for="data in bahan_baku">{{ data.nama_bahan_baku }}</option>
						</select>
						<p class="mt-2 text-xs text-red-500" v-if="validation['pengadaan.' + index + '.id_bahan_baku']">Field ini harus diisi</p>
					</div>
					<div class="col-span-12 sm:col-span-6">
						<label class="label">Qty ({{ pengadaan[index].id_bahan_baku != "" ? "dalam satuan " + pengadaan[index].id_bahan_baku.satuan : "dalam satuan bahan baku" }})</label>
