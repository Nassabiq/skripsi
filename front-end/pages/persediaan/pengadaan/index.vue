		...
		<div class="card">
			<div class="grid grid-cols-12 gap-4">
				...
				<div class="flex items-center justify-between col-span-12 mb-2 space-x-2 md:justify-end md:mb-0 md:col-span-6">
					<div class="">
						<select class="px-2 py-2 text-xs font-medium text-gray-600 border border-gray-200 rounded-lg shadow focus:outline-2 focus:outline-blue-100 focus:ring-2 focus:ring-blue-300" v-model="selectedStatus">
				...
				<table class="table table-auto table-produk">
					...
					<tbody class="divide-y-2 divide-gray-100 divide-dotted" v-for="(data, index) in searchData">
						<tr class="text-sm">
							...
							<td class="p-3">{{ data.tgl_dibuat }}</td>
							<td class="w-48 p-1 py-3">
								<div class="flex flex-wrap gap-2">
									...
									<NuxtLink :to="{name: 'persediaan-pengadaan-id', params: {id: data.id_pengadaan}}">
...
export default {
	...
	computed: {
		searchData() {
			let data = this.pengadaan_barang;

			if (this.search != "" && this.search) {
				data = data.filter((item) => {
					return item.nama_pengadaan.toUpperCase().includes(this.search.toUpperCase());
				});
			}
			...
			if (this.selectedStatus !== "" && this.selectedStatus) {
				...
				data = data.filter((s) => {
					return s.status_pengadaan == this.selectedStatus;
				});
			}
	...
	methods: {
		async getAllData() {
			this.$axios
				.get("/api/pengadaan")
				.then((response) => {
					this.materials = response.data.bahan_baku;
					this.pengadaan_barang = response.data.pengadaan;
				})
				.catch((error) => {
					console.log(error);
				});
