<template>
	<div class="gap-4 space-y-4 margin-auth">
		<div class="card">
			<div class="grid grid-cols-12 gap-4">
				<div class="col-span-5">
					<div class="item">
						<img v-if="product.image" :src="imageUrl + product.slug + '/' + currentImage" class="object-cover w-full border-2 border-slate-200 h-80" />
						<!-- <Skeleton v-else /> -->
						<!-- <p v-else>Delete</p> -->
					</div>
					<div class="grid grid-cols-4 gap-4 mt-4 thumbnails">
						<div class="col-span-1 cursor-pointer" v-for="(img, index) in product.image" :key="index" @click.prevent="activateImage(index)">
							<img :src="imageUrl + product.slug + '/' + img.filename" alt="" :class="activeImage == index ? 'outline outline-offset-2 outline-4 outline-sky-500 ring-2 ring-red-300' : ''" class="object-cover w-full h-24 rounded-md" />
						</div>
					</div>
				...
				<div class="col-span-7">
					...
					<div class="py-4 border-b-2 border-slate-200">
						...
						<p v-if="product.description" class="text-sm text-justify text-slate-500">{{ product.description }}</p>
					...
					<div class="flex items-center justify-end gap-4 mt-4">
						<button class="btn btn-sm btn-with-icon btn-green" @click.prevent="openModal">
						...
						<button class="btn btn-sm btn-with-icon btn-red" @click="deleteProduk">
							<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 20 20" fill="currentColor">
		...
		<Modal size="max-w-xl" title="Edit Data Produk" @close-modal="modal = !modal" v-show="modal">
			...
			<template #submit>
				<button class="btn btn-lg btn-indigo" @click.prevent="updateProduk">
					Update
				</button>
			</template>
		</Modal>
		<Modal size="max-w-2xl" title="Edit Image Produk" @close-modal="modalImage = !modalImage" v-show="modalImage">
			...
			<template #submit>
				<button class="btn btn-lg btn-indigo" @click.prevent="updateImage">
					Update
				</button>
			</template>
		</Modal>
