<template>
	<div class="gap-4 space-y-4 margin-auth">
		<div class="card">
			<div class="grid grid-cols-12 gap-4">
				...
				<div class="flex items-center justify-between col-span-12 mb-2 space-x-2 md:justify-end md:mb-0 md:col-span-6">
					<div class="">
						<button class="btn-with-icon btn btn-sm btn-blue" @click="modalTambahProduk = !modalTambahProduk">
				...
				<table class="table table-auto table-produk">
					...
					<tbody class="divide-y-2 divide-gray-100 divide-dotted">
						<tr class="text-sm" v-for="product in products" :key="product.id">
							<td class="w-32 p-3">
								<img :src="product.image" alt="" class="border-2 rounded-md w-28 border-slate-200" srcset="" />
							</td>
							<td class="p-3">{{ product.nama_produk }}</td>
							<td class="p-3">{{ product.categories.nama_kategori }}</td>
							<td class="p-3">1961</td>
							<td class="p-1">
								<button class="p-1 text-blue-400 border border-blue-600 rounded-full focus:bg-blue-100 hover:text-blue-800" @click.prevent="showRow">
		...
		<div class="grid grid-cols-2 gap-4">
			<div class="col-span-2 card md:col-span-1">
				<div class="my-4">
					<button class="btn-with-icon btn btn-sm btn-blue" @click="modalTambahKategori = !modalTambahKategori">
						<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
						</svg>
				...
				<div class="my-4 overflow-x-auto overflow-y-auto bg-white rounded-lg shadow">
					<table class="table table-auto table-kategori-produk">
						...
						<tbody class="divide-y-2 divide-gray-100 divide-dotted">
							<tr class="text-sm" v-for="category in categories">
								<td class="p-3">1.</td>
			...
			<div class="col-span-2 card md:col-span-1">
				<div class="my-4">
					<button class="btn-with-icon btn btn-sm btn-blue" @click="modalTambahSatuan = !modalTambahSatuan">
						<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
						</svg>
						Tambah Jenis Satuan
		...
		<!-- Modal Tambah Produk -->
		<Modal size="max-w-3xl" title="Tambah Produk" @close-modal="modalTambahProduk = false" v-show="modalTambahProduk">
			<template #content>
		...
		<Modal size="max-w-lg" title="Tambah Kategori" @close-modal="closeModalCategory" v-show="modalTambahKategori">
			<template #content>
				<div class="col-span-12 sm:col-span-9">
				...
				<div class="col-span-12 sm:col-span-3">
					...
					<select class="form-input form-input-lg" v-model="kategori.id_satuan">
						...
						<option :value="unit.id_satuan" v-for="unit in units">{{ unit.nama_satuan }}</option>
					</select>
					<p class="mt-2 text-xs text-red-500" v-if="validation.id_satuan">{{ validation.id_satuan[0] }}</p>
				</div>
			</template>
			<template #submit>
				<button class="btn btn-lg btn-green" @click="addKategori">
					Submit
				</button>
			</template>
		</Modal>
		<!-- Modal Tambah Satuan -->
		<Modal size="max-w-md" title="Tambah Satuan" @close-modal="closeModalUnit" v-show="modalTambahSatuan" ref="modalSatuan">
			...
			<template #submit>
				<button @click="addSatuan" class="btn btn-lg btn-green">
					Submit
				</button>
			</template>
			<!-- </form> -->
...
export default {
	...
	computed: {
		image() {
			...
			for (let i = 0; i < file.length; i++) {
				imageData.append("produk.image[]", file[i]);
			}

			return imageData.getAll("produk.image[]");
		},
	},
	...
	methods: {
		...
		removeImage(key) {
			...
			console.log(this.produk.image);
		},
		closeModalCategory() {
			this.modalTambahKategori = false;
			this.kategori.id_satuan = null;
			this.kategori.nama_kategori = null;

			this.validation.nama_kategori = null;
			this.validation.id_satuan = null;
		},

		closeModalUnit() {
			(this.modalTambahSatuan = false), (this.satuan.nama_satuan = null), (this.validation.nama_satuan = null);
		},
		...
		async addProduk() {
			...
			// 			id_kategori_produk: this.produk.id_kategori_produk,
		...
		// Satuan
		async addSatuan() {
			let modal = this.$refs.modalSatuan;
			console.log(modal);
			// this.isloading = true;
			// setTimeout(() => {
			// 	this.$axios
